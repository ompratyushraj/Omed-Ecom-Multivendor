...
export const updateHomeCategory = createAsyncThunk<
  HomeCategory,
  { homeCategoryId: number; data: HomeCategory }
>(
  "homeCategory/updateHomeCategory",
  async ({ homeCategoryId, data }, { rejectWithValue }) => {
    try {
      const response = await api.patch(`${API_URL}/homeCategory/${homeCategoryId}`, data);
      console.log("category updated", response);
      return response.data;
    } catch (error: any) {
      console.log("error", error);
      if (error.response && error.response.data) {
        return rejectWithValue(error.response.data); // Return error response data if
      } else {
...
export const fetchHomeCategories = createAsyncThunk<HomeCategory[]>(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await api.get(`${API_URL}/homeCategory`);
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(updateHomeCategory.fulfilled, (state, action) => {
      ...
      const index = state.categories.findIndex(
        (category) => category.homeCategoryId === action.payload.homeCategoryId
      );
      if (index !== -1) {
